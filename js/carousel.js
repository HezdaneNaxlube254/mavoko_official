...
function createHeroCarousel() {
    ...
    if (!heroCarousel) {
        return null; // no hero carousel on this page
    }
    ...
    if (prefersReducedMotion) {
        ...
        if (pauseBtn) {
            pauseBtn.addEventListener('click', toggleAutoAdvance);
        }
        ...
        if (carouselContainer) {
            ...
            carouselContainer.addEventListener('mouseleave', function() {
                if (autoAdvance) resumeAutoAdvance();
            });
        }
    ...
    function restart() {
        ...
        // Re‑read slides and dots because CMS may have updated them
        // (the same elements are still present)
        // Reset to first slide
        goToSlide(0);

        // Restart auto‑advance if appropriate
        if (!prefersReducedMotion) {
            startAutoAdvance();
        }
    }
    ...
    carousels.forEach(function(carousel) {
        ...
        if (!autoDisabled && !prefersReducedMotion) {
            ...
            carousel.addEventListener('mouseleave', function() {
                if (autoAdvance && !autoDisabled && !prefersReducedMotion) startAutoAdvance();
            });
        ...
        updateGallery(0);
        if (!autoDisabled && !prefersReducedMotion) startAutoAdvance();
...
document.addEventListener('DOMContentLoaded', function() {
    heroCarouselInstance = createHeroCarousel();
    if (!heroCarouselInstance) {
        // No hero carousel found, still init gallery carousels
        initGalleryCarousels();
    } else {
        // Gallery carousels are also initialised inside createHeroCarousel
        // by calling initGalleryCarousels at the end of that function.
        // So we don't need to call it again here.
    }
});

// Expose the restart function globally for the CMS loader
window.restartHeroCarousel = function() {
    if (heroCarouselInstance && heroCarouselInstance.restart) {
        heroCarouselInstance.restart();
    }
};